...
<body>
    <br> 
  <h1>Supprimer Image</h1>
  <br><br>
  <br><br>
  <center>
  <?php 
		 $x=$_GET['d']; 
$sqlI = "select * from gallerie where id_gal='$x'";
$resultI = mysql_query($sqlI) or die ("Error in query: $query. " . mysql_error());

while($ligneI=mysql_fetch_array($resultI)){
		$id_img = $ligneI['id_gal']  ; 
		$titre = $ligneI['titre']  ; 
		$img = $ligneI['img']  ;


?>		
		<p><font size="+2"> Voulez vous vraiment supprimer cette fichier  </font></p>
		<br>
...
<input type="submit" name="xx" value="  confirmer  ">
  <?php 
  if (isset($_POST['xx'])){
  $sqlD = "delete from gallerie where id_gal='$x'";
